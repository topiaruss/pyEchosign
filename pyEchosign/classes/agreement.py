...
class Agreement(object):
    ...
    Attributes:
        ...
        id (str): The ID assigned to the agreement by Echosign, used to identify the agreement via the API
    ...
    def __init__(self, account: 'eaccount.EchosignAccount', **kwargs):
        ...
        self.id = kwargs.pop('fully_retrieved', None)
        self.name = kwargs.pop('fully_retrieved', None)
        provided_status = kwargs.pop('fully_retrieved', None)
    ...
    def cancel(self):
        ...
        if response_success(r):
            ...
            try:
                log.error('Error encountered cancelling agreement {}. Received message: {}'.format(self.id, r.content))
            finally:
                check_error(r)
    ...
    def delete(self):
        ...
        """
        url = self.account.api_access_point + endpoints.DELETE_AGREEMENT + self.id

        r = requests.delete(url, headers=get_headers(self.account.access_token))

        if response_success(r):
            log.debug('Request to delete agreement {} successful.'.format(self.id))
        else:
            try:
                log.error('Error encountered deleting agreement {}. Received message:{}'.format(self.id, r.content))
            finally:
                check_error(r)
